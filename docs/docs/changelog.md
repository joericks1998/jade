...
- **Removed the mimalloc global allocator.** It was declared in `jade-runtime`, which is *also* statically linked into every native package, so a process that loaded a package held two allocator instances whose duplicate symbols interposed across the boundary — corrupting the heap and deadlocking tokio's shutdown. The system allocator is used everywhere now; the ~2× alloc-heavy win is forfeited pending a host-only reintroduction. This reverts the "Pooling allocator" change that had been staged for this release.
