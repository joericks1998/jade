...
- **A tripwire test binds the compiler to that definition.** The compiler cannot `use` a `.jde` in its own Rust and C sources, so it keeps a hand-written copy of the field list. A test parses `protocol/jade/infer.jde` with the compiler's own lexer and parser — not a regex, so a comment naming a field cannot fool it — and fails on any difference, naming both lists. A second test reads the C emitter's source and asserts it names every field, since a Rust constant cannot reach it. A missing submodule is a hard failure rather than a skip: an absent definition is exactly when drift goes unnoticed.
- **Breaking for provider packages, again.** A package written against the v1.1.30 dict receives a struct and reads `request["prompt"]` as nothing. Providers must read `request.input` and be rebuilt. `jade build --lib` output from any earlier version needs the same.
