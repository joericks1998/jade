...
- **The repository root holds only the things that must live there.** The Cargo build script moved to `src/runtime_aot/build.rs`, beside the C runtime it compiles, declared through `build =` in `Cargo.toml` — it could not go to `src/build.rs`, since `src/build/` already claims that module name. And the root `install.sh` is gone: it was a byte-for-byte duplicate of `docs/static/install.sh`, which is the copy actually served at `jadelang.org/install.sh`. Nothing synced the two, so they matched only as long as every edit remembered both. No change to how anything builds or installs.
