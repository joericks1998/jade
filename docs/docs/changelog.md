...
- **Performance.** Several backend/runtime optimizations with no language-visible change:
  ...
  - **Pooling allocator.** Route the Rust heap path (collection object boxes + their `Vec`/`HashMap`) through mimalloc, which grabs large arenas up front and recycles per-thread. Allocation-heavy programs ~2× faster; no effect on non-allocating code.
