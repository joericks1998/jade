...
- **`mixed-type-containers.md`** — why an array literal could not hold two types, and why the inference response wanted one that could. Mixed arrays shipped in v1.1.32; a named sum type has not. Worth reading for what removing the restriction cost, which was nothing in the type checker and three engine divergences the restriction had been hiding. Read it before adding a sum type.
