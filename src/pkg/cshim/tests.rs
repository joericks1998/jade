...
fn wrappers_check_arity_and_tags_before_calling() {
    ...
    assert!(src.contains("if (argv[0].tag != JADE_FFI_INT) {"), "missing tag check:\n{src}");
    // A pointer parameter takes nil as NULL, so the check admits both tags and
    // nothing else. `g_uri_escape_string(s, NULL, TRUE)` is an ordinary call.
    assert!(
        src.contains("if (argv[1].tag != JADE_FFI_STR && argv[1].tag != JADE_FFI_NIL) {"),
        "missing tag check:\n{src}"
    );
    assert!(
        src.contains(
            "(f)((int64_t)argv[0].data.as_int, \
             (argv[1].tag == JADE_FFI_NIL ? NULL : argv[1].data.as_str))"
        ),
        "bad call:\n{src}"
    );
}
...
fn rejects_an_unrepresentable_argument_type() {
    ...
    // All three positions, not just the one that failed: writing a return type
    // in `args` is a common enough mistake that seeing where the spelling *is*
    // legal answers the question in one read.
    assert!(err.contains("In `args`:"), "error should list what works: {err}");
    assert!(err.contains("In `ret`:"), "error should list return types too: {err}");
    // The list is the definitive one or it is worse than nothing — someone
    // hand-writing a stanza reads it and concludes the type they need does not
    // exist. These six were generated by `jade pkg add` and named nowhere.
    for t in ["bytes_ptr", "sized_buffer", "inout_bytes", "callback", "null_ptr", "out_str"] {
        assert!(err.contains(t), "vocabulary omits `{t}`, which the generator writes: {err}");
    }
}
...
fn a_symbol_that_cannot_fail_does_not_touch_errno() {
    ...
    // A tag check raises too, and should — it is how a caller learns it passed
    // the wrong kind of value. What must not appear is the *convention's* raise,
    // which is the one that reads errno.
    assert!(!src.contains("jade_shim_errmsg"), "nothing should raise a failure:\n{src}");
}
...
fn an_input_blob_becomes_a_pointer_and_a_length() {
    ...
    assert!(src.contains("if (argv[0].tag != JADE_FFI_BYTES) {"), "missing tag check:\n{src}");
...
fn a_blob_with_no_length_becomes_one_pointer() {
    ...
    assert!(src.contains("if (argv[0].tag != JADE_FFI_BYTES) {"), "no tag check:\n{src}");
...
fn two_revised_blobs_free_both_when_the_call_raises() {
    ...
    // Anchored on the convention's raise, not on the first `JADE_FFI_ERROR` in
    // the function — an argument that fails its tag check raises before the call
    // is even made, and nothing is allocated yet for it to free.
    let raise = src.split("out->data.as_str = jade_shim_errmsg();").next().unwrap_or_default();
...
fn a_struct_input_takes_one_jade_argument() {
    ...
    assert!(src.contains("if (argv[0].tag != JADE_FFI_STRUCT) {"), "no tag check:\n{src}");
}
...
fn a_caller_sized_buffer_allocates_what_it_was_asked_for_and_hands_it_all_back() {
    ...
    .unwrap();
    assert!(src.contains("if (argv[0].tag != JADE_FFI_INT) {"), "no count check:\n{src}");
...
fn a_returned_string_is_lent_when_the_library_keeps_it_and_released_when_it_does_not() {
    ...
    // Both are copied. The pointer's lifetime is never the shim's to assume —
    // a return may point into a buffer this wrapper is about to free, which is
    // what `uriEscapeA` does. What separates the two spellings is the release.
    let lent = generate("glib", &symbols(&[("f", sym(&["str"], "str"))])).unwrap();
    assert!(lent.contains("jade_shim_owned(r)"), "should still copy:\n{lent}");
    assert!(lent.contains("out->data.as_str = r_c;"), "should hand back the copy:\n{lent}");
    // The library keeps it, so nothing here may free it.
    assert!(!lent.contains("free(r)"), "the library's string is not ours to release:\n{lent}");
...
#[test]
fn an_inout_struct_is_seeded_from_its_argument_and_handed_back() {
    // The `init` / `update` / `final` shape. Bound `out_struct` these take no
    // argument at all, so every call gets a fresh zeroed state and the previous
    // call's work is discarded — while every call still reports success. That
    // is how libsodium's streaming SHA-256 returned the digest of a state that
    // had never been initialised.
    let s = sym(&["inout_struct:st"], "scalar:int");
    let src =
        generate_with("sod", &symbols(&[("update", s)]), &[("st", &[("count", "int")])], &["s.h"])
            .unwrap();

    // Seeded, not memset to zero: the caller's state is read into the local.
    assert!(src.contains("jade_shim_field"), "should read the state in:\n{src}");
    assert!(src.contains("&iostruct0"), "should pass the state by pointer:\n{src}");
    // And handed back, so the next call in the sequence has something to pass.
    assert!(src.contains("iostruct0_j"), "should hand the state back:\n{src}");
    // It takes a Jade argument, which `out_struct` does not.
    assert!(src.contains("if (argc != 1) return 1;"), "should consume an argument:\n{src}");
    // Writable, because the library advances it.
    assert!(!src.contains("const st*"), "must not be const:\n{src}");
}

#[test]
fn a_returned_string_is_copied_before_a_buffer_it_may_point_into_is_freed() {
    // `uriEscapeA` fills the caller's buffer and returns a pointer to the
    // terminating NUL *of that buffer*. The header cannot say so, and neither
    // can the person writing the manifest — so the shim does not ask. It copies
    // first, which is right whether the return aliased or not.
    let s = sym(&["str", "sized_buffer:char", "scalar:int", "scalar:int"], "str");
    let src = generate("uriparser", &symbols(&[("uriEscapeA", s)])).unwrap();

    // Two results means a struct, and a string inside a container is
    // container-owned — so the copy is a `strdup` that `ffi_free` reclaims,
    // rather than the borrowed per-thread buffer a lone return would get.
    let copy = src.find("strdup((const char*)r)").expect("the return should be copied");
    let release = src.find("free(sbuf1);").expect("the buffer should still be freed");
    assert!(copy < release, "the copy must precede the free:\n{src}");
    // And the copy is what Jade is handed, not the pointer into freed memory.
    assert!(src.contains("data.as_str = r_c;"), "should hand back the copy:\n{src}");
}
...
fn an_owned_string_return_is_released_on_the_failure_path_too() {
    ...
    // The convention's raise, not a tag check's — see the note in
    // `two_revised_blobs_free_both_when_the_call_raises`.
    let fail_at = src.find("out->data.as_str = jade_shim_errmsg();").expect("a failure branch");
...
fn a_handle_and_a_scalar_keep_their_argument_positions() {
    ...
    assert!(
        src.contains(
            "(f)((int64_t)argv[0].data.as_int, (sqlite3*)h1, \
             (argv[2].tag == JADE_FFI_NIL ? NULL : argv[2].data.as_str))"
        ),
        "bad call:\n{src}"
    );
}
...
fn an_inout_scalar_consumes_an_argument_and_is_seeded_from_it() {
    ...
    assert!(src.contains("if (argv[1].tag != JADE_FFI_INT) {"), "missing tag check:\n{src}");
...
fn a_named_c_type_is_declared_and_converted_at_the_boundary() {
    ...
    assert!(src.contains("if (argv[1].tag != JADE_FFI_INT) {"), "no tag check:\n{src}");
    assert!(
        src.contains(
            "(g_uri_escape_string)((argv[0].tag == JADE_FFI_NIL ? NULL : argv[0].data.as_str), \
             (int)argv[1].data.as_int)"
        ),
