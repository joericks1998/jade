//! Consolidated unit tests for the `compiler` module.
//!
//! Relocated from `vm_tests.rs`, `type_infer_tests.rs`, and the inline
//! `gbnf` test block, plus new tests for `emit`, `bytecode`, and `tir`.
//! One inline submodule per source module keeps each test scope clean.
#![allow(clippy::all)]
...
mod bytecode {
    use crate::compiler::bytecode::{Chunk, Instr};
    ...
    fn intern_fn_returns_sequential_indices() {
        use crate::compiler::bytecode::CompiledFn;
        use std::sync::Arc;
        let mut c = Chunk::new("t");
