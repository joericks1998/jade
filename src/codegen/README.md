...
*A call site cannot know a callee's async-ness either.* Whether a call starts a task was decided here, from the static type of what was being called, and a value has no static type. So holding an `async fn` in a local, a collection, or a return refused to build outright ("spawn of a non-static function"), and an `async fn` imported from a module was devirtualized into a direct call that ran the body inline. The box carries the fact instead (`OBJ_FN_ASYNC`, mirroring `JRT_FN_ASYNC`), a spawn with no static callee lowers through `jrt_call_value` like any other indirect call, and a call is *not* devirtualized when its target is async. The same path carries a wrong argument count to the callee's own entry, so it raises rather than refusing to build.
...
*A read and the release it pairs with belong in one call.* A borrowed read — `Move`, `GetLocal`, an element — retains for the destination, and the store then releases whatever the destination held. Written as `retain` beside `store` those drifted apart, because `jrt_rc_replace` skips the release when the slot already holds that very object. The skip is right for an in-place write-back, which hands back the object the slot already has and took no reference; it is wrong after a retain, which is then unbalanced. `store_borrowed` does both, `rc_replace_slot_retained` is the always-release half, and the plain `rc_replace_slot` keeps the skip for the write-back case — `examples/bytes/buffer` double-frees within seconds without it.

Anything that stores into a slot must go through one of those two, never `slot_store` alone: the spill array is kept in step inside them, and a store that bypasses it leaves the spill stale, so a frame unwound past releases the wrong thing or nothing. That is how the first version of `store_borrowed` turned `examples/exceptions/raise_through_frames` from 1 live object into 152.

*Every frame leaves a copy of what it holds, and it is not the register file.* A `longjmp` runs nothing on the way out, so a skipped frame never releases what its registers hold. Each frame therefore hands the runtime a *spill* array in its prologue, and the throw releases the spills of the frames it erases (`jade_frame_release_above`).

Handing over the registers themselves is the obvious design and it is the wrong one: it costs 41% on `bench/extreme.jde`, because an escaping register file cannot be promoted out of memory and every register access becomes a load. The spill escapes instead and the registers do not. It is written in exactly one place — inside `rc_replace_slot`'s heap branch, which already runs only when a heap word is involved — so arithmetic-only code pays nothing to keep it in step. A scalar written over a scalar skips the branch and leaves a stale scalar in the spill, which releases as nothing.

The ownership rule this rests on is that *a store takes ownership*: `jrt_rc_replace` releases the slot's old value and does not retain the new one. So a value has one owner, the register holding it, and releasing that register releases the value. Which is why `throw` retains before any of this runs, and why the landing pad binds the caught value with no retain of its own — it is taking the exception's reference. Those two and the frame release move together; none works alone.

Two things keep the remaining cost near zero. A frame only registers when `exc_depth != 0`, because a handler installed deeper cannot unwind past something shallower and a raise with no handler ends the process — so a frame entered outside every `try` can never be skipped. And the spill is filled by `jrt_recur_enter` on that same gated path rather than in the prologue, so a call outside a `try` does not pay N stores to initialise something nothing will read. A landing pad puts its own frame *back* on the registry (`emit_recur_resume`), because a catch is not a return and the handler body goes on using those registers.

*Three counters unwind at an exception landing pad, not two.* A `longjmp` skips every intervening frame's epilogue, so whatever those frames left open is still open when the catch runs. The recursion counter and the handler stack were already restored there. The generator stack is the third: a `yield`ing function opens a buffer on entry and closes it on return, a raise in between never reaches the close, and the caller's next `yield` then landed in the abandoned buffer — `fn outer() { yield 1; try { inner() } catch e { }; yield 2 }` gave `[10, 2]` compiled against `[1, 2]` interpreted. `emit_yield_restore` truncates back to the depth snapshotted in the prologue, which is taken *after* the generator's own `jrt_yield_push` so a generator that catches its own raise keeps its own buffer. If you add a fourth thing a function opens and closes, it belongs in the same list.
