...
*A raise that leaves its function runs the function's scope exit first.* A `longjmp` runs nothing on the way out, so a skipped frame never releases what its registers hold, and for the raiser that includes the raised value. The raiser is the one frame that knows it is leaving, so `Raise` emits the same scope exit a `return` does — after retaining the value, since one of the registers it is about to release is holding it. That fixes the ownership question too: the throw takes a reference of its own and the `catch` binding takes *that* reference, with no retain at the landing pad. All three parts move together and none works alone.

Not when a handler of this same function is active, which is what `BodyCtx::in_handler` answers: the frame is staying and the catch arm goes on using those registers. A frame merely *unwound past* still leaks its locals. Fixing that means handing the runtime a base pointer to the register file so a throw can walk it, which was measured at 41% on `bench/extreme.jde` — an escaping register file cannot be promoted out of memory, so every register access becomes a load.
