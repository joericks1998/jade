...
- **`builtins.rs`** — builtin dispatch: native package calls, stdlib module calls, primitive value methods, and runtime-dispatched struct methods. The `chunk_*_supported` predicates that gate them live here too.
  ...
  **A name they answer `false` for is a hard build error**, so the set they accept has to match what `jade run` accepts. It did not: `string.upper(s)` ran and refused to build until v1.3.21, while the identical `s.upper()` compiled. `as_receiver_first` closes that by lowering the package spelling as the method — but only for names the package table actually lists, and only for `std/string` and `std/dict`, where the two spellings are the same function. `std/array` is deliberately not: `array.sort(a)` returns a copy and `a.sort()` mutates, so those get their own lowering over `jrt_coll_array_sorted`. Read `package_fn_is_the_method` before widening it.

  It also holds `check_globals_bound`, the one whole-program check this directory runs before emitting anything. Reading a global nothing binds has no lowering to decline — `global_slot` creates the cell, initialized to nil, and the program builds — so an undefined name produced a binary that ran and then died in the runtime with no message. Type inference catches most of these; what reaches here is what it has to stay lenient about, a file importing a user module. By this point every import is inlined, so the program's own bindings plus the runtime's are all there is, and the question is finally answerable. Read it before adding a global that nothing `SetGlobal`s: type names are already exempt because they live in the side tables rather than the instruction stream.
