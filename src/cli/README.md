...
- **`pkg.rs`** — `jade pkg add` / `bind` / `remove` / `install` / `update` / `list`. `bind` reads a C dependency's header with clang and writes its symbol table into `jade.toml`, so a library with hundreds of entry points does not have to be transcribed by hand; it prints what it could not bind, and why, because a generator that silently covers most of an API is how the rest is found at run time. `--dry-run` shows that report without touching the manifest. The manifest is the source of truth and `jade.lock` is derived from it; with no registry to query, "update" means reconciling the lock with the manifest, not discovering a newer version. `install` re-hashes local `path` dependencies before installing, since those point at files the user rebuilds; `--locked` reports that drift as an error instead.
