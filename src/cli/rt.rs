...
// ── Bundled C sources (embedded at compile time) ──────────────────────────────

const RT_HEADER:  &str = include_str!("../../jade_rt/jade_rt.h");
const RT_PTHREAD: &str = include_str!("../../jade_rt/jade_rt_pthread.c");
const RT_JADEOS:  &str = include_str!("../../jade_rt/jade_rt_jadeos.c");
...
pub fn run_rt_build(target: &str, output: Option<&str>, cc: &str, ar: &str) {
    ...
    std::fs::write(&header_path, RT_HEADER).unwrap_or_else(|e| {
        eprintln!("error: {}", e); process::exit(1);
    });
    std::fs::write(&src_path, if jadeos { RT_JADEOS } else { RT_PTHREAD })
...
fn make_scratch_dir() -> Result<PathBuf, std::io::Error> {
    // jade-rt-<pid> inside the system temp dir — unique enough without a crate.
