...
*A throw gives back what the frames it erases were holding.* A `longjmp` runs nothing on its way out, so every frame it skips loses its scope-exit releases. Each frame leaves a *spill* array behind for exactly this — a copy of whatever its registers hold that is worth releasing — and `jade_frame_release_above` walks the spills of the frames above the handler's own depth, before the jump, while those frames still exist. Afterwards they sit below the stack pointer and reading them is not allowed. Deliberately not the registers themselves: see `src/codegen/README.md` for the 41% that costs. Registering is gated on `exc_depth != 0`, because a frame entered outside every `try` can never be skipped.

*A binary's output streams, and one print is one write.* Two things a service needs that a terminal hides. stdio picks full buffering for a stream that is not a terminal, so a binary logging to a file or a supervisor produced nothing until 4 KB had piled up and lost the rest on SIGTERM — Rust line-buffers stdout regardless, so the interpreter never had it, and a `setvbuf` in a constructor gives a binary the same. And a print was several `fputs` calls with nothing between them, so concurrent tasks spliced their text: four tasks printing 200 lines each gave 56 corrupt lines out of 800. One lock spans the whole of a print, which is why the body lives in `jrt_print_any_locked` and `write` shares it rather than taking the lock twice.

*A function box says whether calling it starts a task.* `JRT_FN_ASYNC`, above the sub-kind byte, is what makes an `async fn` a value: a call site holding only the value has no static type to decide from, so `jrt_call_value` reads the bit and spawns. The task is handed the *box word*, not the raw entry, so what the pool retains is an ordinary tagged value; `jade_task_entry_shim` unwraps it and enters the body directly, because coming back through `jrt_call_value` would see the bit again and spawn a second task.
