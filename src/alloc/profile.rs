//! Allocation profiler — Phase 0 of the allocation remediation (`--features
//! alloc-profile`). Wraps the system allocator and records a size-class
//! histogram plus live/peak bytes, dumped to stderr at process exit.
//!
//! HOST-ONLY BY CONSTRUCTION. This is declared as the `jade` *binary's*
//! `#[global_allocator]` in `main.rs`, never in the shared `jade-runtime` crate,
//! so — unlike mimalloc — it cannot be linked into a dlopen'd package and create
//! two allocator instances. It exists only to measure the VM's allocation
//! profile so the Phase-1 pool's size classes are chosen from data, not guessed.
//! It is never built into a release.
//!
//! Run e.g.:
//!   cargo build --features alloc-profile
//!   JADE_ALLOC_PROFILE=1 ./target/debug/jade run bench/alloc_heavy.jde

use std::alloc::{GlobalAlloc, Layout, System};
use std::sync::atomic::{AtomicU64, Ordering::Relaxed};

/// Power-of-two size buckets: index `b` counts allocations of `2^(b+2)..2^(b+3)`
/// bytes, i.e. bucket 0 is 4–7 bytes, bucket 1 is 8–15, … bucket 21 is ≥16 MiB.
const NBUCKETS: usize = 22;

static COUNTS: [AtomicU64; NBUCKETS] = [const { AtomicU64::new(0) }; NBUCKETS];
static BYTES:  [AtomicU64; NBUCKETS] = [const { AtomicU64::new(0) }; NBUCKETS];
static TOTAL_ALLOCS: AtomicU64 = AtomicU64::new(0);
static TOTAL_FREES:  AtomicU64 = AtomicU64::new(0);
static LIVE_BYTES:   AtomicU64 = AtomicU64::new(0);
static PEAK_BYTES:   AtomicU64 = AtomicU64::new(0);

/// Bucket index for `size`: `floor(log2(size)) - 2`, clamped to `[0, NBUCKETS)`.
#[inline]
fn bucket(size: usize) -> usize {
    let log2 = (usize::BITS - 1 - size.max(1).leading_zeros()) as usize;
    log2.saturating_sub(2).min(NBUCKETS - 1)
}

pub struct ProfilingAlloc;

unsafe impl GlobalAlloc for ProfilingAlloc {
    unsafe fn alloc(&self, layout: Layout) -> *mut u8 {
        let p = unsafe { System.alloc(layout) };
        if !p.is_null() {
            let size = layout.size();
            let b = bucket(size);
            COUNTS[b].fetch_add(1, Relaxed);
            BYTES[b].fetch_add(size as u64, Relaxed);
            TOTAL_ALLOCS.fetch_add(1, Relaxed);
            let live = LIVE_BYTES.fetch_add(size as u64, Relaxed) + size as u64;
            // Monotone peak; the read-then-max race only ever under-reports
            // slightly, which is fine for a profiler.
            if live > PEAK_BYTES.load(Relaxed) {
                PEAK_BYTES.store(live, Relaxed);
            }
        }
        p
    }

    unsafe fn dealloc(&self, ptr: *mut u8, layout: Layout) {
        TOTAL_FREES.fetch_add(1, Relaxed);
        LIVE_BYTES.fetch_sub(layout.size() as u64, Relaxed);
        unsafe { System.dealloc(ptr, layout) };
    }

    unsafe fn realloc(&self, ptr: *mut u8, layout: Layout, new_size: usize) -> *mut u8 {
        let p = unsafe { System.realloc(ptr, layout, new_size) };
        if !p.is_null() {
            // Model a realloc as a free of the old block + an alloc of the new one
            // so the histogram reflects the sizes actually requested.
            TOTAL_FREES.fetch_add(1, Relaxed);
            LIVE_BYTES.fetch_sub(layout.size() as u64, Relaxed);
            let b = bucket(new_size);
            COUNTS[b].fetch_add(1, Relaxed);
            BYTES[b].fetch_add(new_size as u64, Relaxed);
            TOTAL_ALLOCS.fetch_add(1, Relaxed);
            let live = LIVE_BYTES.fetch_add(new_size as u64, Relaxed) + new_size as u64;
            if live > PEAK_BYTES.load(Relaxed) {
                PEAK_BYTES.store(live, Relaxed);
            }
        }
        p
    }
}

/// Format the accumulated histogram. Printed to stderr from `main` at exit when
/// `JADE_ALLOC_PROFILE` is set (so ordinary `--features alloc-profile` runs stay
/// quiet unless asked).
pub fn report() -> String {
    let allocs = TOTAL_ALLOCS.load(Relaxed);
    let frees = TOTAL_FREES.load(Relaxed);
    let peak = PEAK_BYTES.load(Relaxed);
    let mut out = String::new();
    out.push_str("\n── jade allocation profile ─────────────────────────────\n");
    out.push_str(&format!(
        "total allocations: {allocs}\ntotal frees:       {frees}\npeak live bytes:   {peak}\n\n"
    ));
    out.push_str("size-class          count        bytes    %count\n");
    for b in 0..NBUCKETS {
        let c = COUNTS[b].load(Relaxed);
        if c == 0 {
            continue;
        }
        let bytes = BYTES[b].load(Relaxed);
        let lo = 1usize << (b + 2);
        let hi = (1usize << (b + 3)) - 1;
        let pct = if allocs > 0 { c as f64 * 100.0 / allocs as f64 } else { 0.0 };
        out.push_str(&format!("{lo:>7}–{hi:<9} {c:>10} {bytes:>12} {pct:>8.1}%\n"));
    }
    out.push_str("────────────────────────────────────────────────────────\n");
    out
}

#[cfg(test)]
mod tests {
    use super::*;

    // These only build under `--features alloc-profile`, since the module does:
    //     cargo test --features alloc-profile alloc::profile
    //
    // `ProfilingAlloc` is *not* the global allocator in the test binary — that
    // declaration lives in `main.rs`, which the library test target does not
    // compile — so the counters move only when a test calls these methods
    // directly. That is what makes the delta assertions below exact.

    #[test]
    fn bucket_spans_powers_of_two_from_four() {
        // Bucket b covers 2^(b+2)..2^(b+3)-1, so bucket 0 is 4–7.
        assert_eq!(bucket(4), 0);
        assert_eq!(bucket(7), 0);
        assert_eq!(bucket(8), 1);
        assert_eq!(bucket(15), 1);
        assert_eq!(bucket(16), 2);
    }

    #[test]
    fn bucket_clamps_at_both_ends() {
        // Sizes below the first bucket are not a separate case in the histogram;
        // they fold into bucket 0 rather than underflowing the `- 2`.
        assert_eq!(bucket(0), 0, "zero must not underflow the shift");
        assert_eq!(bucket(1), 0);
        assert_eq!(bucket(3), 0);
        // And anything enormous saturates rather than running off the array.
        assert_eq!(bucket(1 << 40), NBUCKETS - 1);
        assert_eq!(bucket(usize::MAX), NBUCKETS - 1);
    }

    /// Serializes every test that allocates through [`ProfilingAlloc`].
    ///
    /// The counters are process-global, so two such tests running in parallel
    /// each see the other's allocations and the delta assertions below flake.
    /// Poisoning is ignored deliberately: one failing test should report its own
    /// assertion, not cascade a confusing poison error into the next one.
    static COUNTERS: std::sync::Mutex<()> = std::sync::Mutex::new(());

    fn counter_guard() -> std::sync::MutexGuard<'static, ()> {
        COUNTERS.lock().unwrap_or_else(|e| e.into_inner())
    }

    #[test]
    fn accounting_tracks_allocs_frees_and_live_bytes() {
        let _guard = counter_guard();
        let a = ProfilingAlloc;
        let small = Layout::from_size_align(4096, 8).unwrap();
        let large = Layout::from_size_align(8192, 8).unwrap();

        let allocs0 = TOTAL_ALLOCS.load(Relaxed);
        let frees0 = TOTAL_FREES.load(Relaxed);
        let live0 = LIVE_BYTES.load(Relaxed);
        let counted0 = COUNTS[bucket(4096)].load(Relaxed);
        let bytes0 = BYTES[bucket(4096)].load(Relaxed);

        unsafe {
            let p = a.alloc(small);
            assert!(!p.is_null());
            assert_eq!(TOTAL_ALLOCS.load(Relaxed), allocs0 + 1, "alloc not counted");
            assert_eq!(LIVE_BYTES.load(Relaxed), live0 + 4096, "live bytes not raised");
            assert_eq!(COUNTS[bucket(4096)].load(Relaxed), counted0 + 1, "histogram missed it");
            assert_eq!(BYTES[bucket(4096)].load(Relaxed), bytes0 + 4096);
            assert!(PEAK_BYTES.load(Relaxed) >= LIVE_BYTES.load(Relaxed), "peak must bound live");

            // A realloc is modelled as a free plus an alloc, so that the histogram
            // reflects the sizes actually requested rather than hiding a growth.
            let q = a.realloc(p, small, large.size());
            assert!(!q.is_null());
            assert_eq!(TOTAL_ALLOCS.load(Relaxed), allocs0 + 2, "realloc should count an alloc");
            assert_eq!(TOTAL_FREES.load(Relaxed), frees0 + 1, "realloc should count a free");
            assert_eq!(LIVE_BYTES.load(Relaxed), live0 + 8192, "live bytes should track the new size");

            a.dealloc(q, large);
            assert_eq!(TOTAL_FREES.load(Relaxed), frees0 + 2);
            assert_eq!(LIVE_BYTES.load(Relaxed), live0, "live bytes should return to the starting value");
        }
    }

    #[test]
    fn report_renders_its_header_and_columns() {
        // Structural only. The numbers move as the test above allocates, so
        // asserting on them here would be a race.
        let out = report();
        assert!(out.contains("jade allocation profile"), "missing title");
        assert!(out.contains("total allocations:"), "missing alloc total");
        assert!(out.contains("total frees:"), "missing free total");
        assert!(out.contains("peak live bytes:"), "missing peak");
        assert!(out.contains("size-class"), "missing histogram header");
    }

    #[test]
    fn report_lists_a_class_that_was_actually_used() {
        // A distinctive size nothing else in the suite requests, so its row is
        // this test's own doing. Bucket 12 spans 16384–32767.
        let _guard = counter_guard();
        let a = ProfilingAlloc;
        let l = Layout::from_size_align(20_000, 8).unwrap();
        unsafe {
            let p = a.alloc(l);
            assert!(!p.is_null());
            a.dealloc(p, l);
        }
        assert_eq!(bucket(20_000), 12);
        assert!(
            report().contains("16384–32767"),
            "a used size class should appear as a histogram row"
        );
    }
}
