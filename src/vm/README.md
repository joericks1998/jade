...
- *`async_tasks.rs`* holds the `JadeFuture` and token-stream handle types, the slot gate that enforces `max_tasks`, plus the task body a spawned task runs on its own `VmState`. The `spawn`, `await`, and `join` opcodes are dispatched inline in `dispatch.rs`, because they manipulate register slots directly.
...
*Depth costs futures, width costs threads.* A task body stays a runtime task rather than becoming a thread, which is what makes a deep `await` chain free: `examples/async/deep_nesting` nests 2,000 levels, and each parked level is a suspended future. Giving each body a blocking thread of its own — the obvious way to let a blocking task run wide — deadlocked that same fixture the moment the chain outgrew the thread pool, which is the trap to know about before rearranging any of this.

What lets a blocking task run wide instead is `async_tasks::blocking`, which the builtins that wait on the outside world call around the wait: `http`, `uhttp`, `sh`, `time.sleep`, `input`, and `fs.read_stdin_bytes`. It is tokio's `block_in_place`, which moves the worker's other work elsewhere for the duration. The ordinary file calls are deliberately not on that list — they finish at disk speed, and paying for the handoff per call would tax a read loop for nothing. Inference is not on it either, because `ProviderPackageBackend::infer` already runs on `spawn_blocking`. Without it the VM ran one task per core whatever `max_tasks` said, so `set_max_tasks(32)` on an eight-core laptop was a promise only the compiled engine could keep — sixteen concurrent requests took two waves under `jade run` and one from the same program built. It exists only on the multi-threaded runtime that `jade run` builds, so the helper checks the flavor first; the unit tests use a current-thread runtime, where it would panic and there is no worker to free anyway.

*The limit is enforced by hand, and `await` gives its slot back.* There is no pool here to size, so `async_tasks` keeps a count and compares it against `jade_runtime::task::max_tasks()` on every decision — a fixed set of semaphore permits could not be resized, and `set_max_tasks` has to take effect on the next task rather than the next run. `parked` wraps every point the interpreter waits on another task, so a task blocked in `await` is not counted as running. Skip that and `set_max_tasks(1)` deadlocks the moment one task awaits another. It is the same rule the compiled pool applies through its `blocked` count, for the same reason.

The flag saying whether a task holds a slot is a *task*-local, not a thread-local. A task may resume on a different worker after every await point, so a thread-local would answer for whichever task ran there last.
