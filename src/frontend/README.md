...
**Structural rules that need a counter are enforced here, not in the type checker.** `return` and `yield` need an enclosing function; `break` and `continue` need an enclosing loop. All four are settled by a depth counter on the parser — `fn_depth` and `loop_depth` — because the answer depends only on where the statement sits, and by type-inference time the nesting has already been flattened into a tree that no longer says it.

`loop_depth` is **reset** across a function boundary rather than merely saved, in all three places a body is parsed: `fn`, `async fn`, and a closure. A loop outside the enclosing function is not one its body can break out of, since leaving it would mean crossing a call frame — which is what `return` is for. Getting that wrong is silent: the parse succeeds and the emitter produces a jump to an address in another chunk.
